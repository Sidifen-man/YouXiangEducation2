...
export default {
  ...
  methods: {
    ...
    next() {
      ...
      this.width.width = this.division*(this.pageSize-1)+'%';
      this.postion.left = this.division*(this.pageSize-1)+'%';
      this.spanPostion.left = this.division*(this.pageSize-1) + 10 + "%";
    ...
    submit(){
      ...
      surver.submitoNet(param).then(res=>{
        ...
        if(res.data.code === "success")this.isShow = 5;
      })
    },
    getoNet(){
      let param = {
