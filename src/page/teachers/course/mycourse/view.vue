<template>
  <div class="mycourseview" v-if="showData" >
    <LoadingPage :isShowModal="loading"></LoadingPage>
    <breadcrumb :address="address"/>

    <div class="view">
      <div class="des">
        <el-row>
          <el-col :span="24">
            <el-row :gutter="30">
              <el-col :span="6">
                <div class="workimg">
                  <img width="188" height="198" :src="courseDetail.imgPath" />
                </div>
              </el-col>
              <el-col :span="18">
                <div class="workdes">
                  <h3 v-html="courseDetail.classCourseName">{{courseDetail.classCourseName}}</h3>
                  <span style="cursor: pointer;" @click="()=>{this.isShowStudentPreview = true}" class="previewstudents">预览学生效果</span>
                  <p v-if="!desState" >
                    {{fitlerdes()}}
                    <var v-if="courseDetail.classCourseDesc.length >　135" @click="()=>{this.desState = true}">查看详情</var>
                  </p>
                  <p v-if="desState">
                    {{courseDetail.classCourseDesc }}
                    <var v-if="courseDetail.classCourseDesc.length >　135"  @click="()=>{this.desState = false}">收起详情</var>
                  </p>

                  <div>
                    <span>开始时间： {{courseDetail.startTime}}</span>
                    <span>结束时间： {{courseDetail.endTime}}</span>
                  </div>
                </div>
              </el-col>
            </el-row>
          </el-col>
        </el-row>
      </div>
      <div class="workList">

        <el-row :gutter="10" v-for="(item, index) in courseDetail.classChapters" :key="item.id">
          <el-col :span="12" class="workList-item">
            <div class="li nowork" v-if="item.blockStatus == '0'">
              <h4>第{{index + 1}}课时：{{item.name}}</h4>
              <div class="icon_no" >
                <div>
                  <i></i>
                  <span>已禁用课件</span>
                </div>

              </div>

              <div class="myteam nowork_myteam">
                <span class="i" @click="handleGrouping(item.id)">小组分配</span>
                <span class="classstart" @click="handleBeginClass(item.id)">开始上课</span>
                <span class="classdata" @click="handlePrepareLesson(item.id)">备课资料</span>
              </div>

            </div>

            <div class="li liLeft" v-if="item.blockStatus == '1'">
              <h4>第{{index + 1}}课时：{{item.name}}</h4>
              <div class="workdes">
                <span @click="openNewPage('/courseware/'+item.id+'?type=teacherTask')">{{item.attachname | filterWord}}<i v-if="item.coursewareStatus!='1'"></i></span>


                <var @click="handleOpenCourseware(item.id)">开放课件 ></var>

                <var tag="var" style="margin-left: 0.2rem;" v-if="item.coursewareStatus!='1'" @click="openNewPage('/courseware/'+item.id+'?type=teacherTaskDelete')">编辑课件 ></var>
                <var tag="var" style="margin-left: 0.2rem;" v-else  @click="openNewPage('/courseware/'+item.id+'?type=teacherTask')">查看课件 ></var>

              </div>
              <div class="myteam">
                <span class="i" @click="handleGrouping(item.id)">小组分配</span>
                <span class="classstart" @click="handleBeginClass(item.id)">开始上课</span>
                <span class="classdata" @click="handlePrepareLesson(item.id)">备课资料</span>
              </div>


            </div>

          </el-col>
          <el-col :span="12" class="workList-item">
            <div class="li">
              <h4>相关任务</h4>

              <div class="tasks">

                <ul>
                  <li v-for='(task, taskIndex) in item.taskShowList' :key='task.id'>
                    <el-row>
                      <el-col :span="11">
                        <h5 :class="task.acttype + '-task'">{{task.name?splitStringData(task.name,11):''}}</h5>
                      </el-col>
                      <el-col :span="7">
                        <small >截止时间：{{(item.coursewareStatus=='1' && item.chapterStatus=='1')?task.datetoStr:''}}</small>
                      </el-col>
                      <el-col :span="6">
                        <span @click="showDetail(task,item.coursewareStatus)" >查看 ></span>
                      </el-col>
                    </el-row>
                  </li>
                </ul>

                <div v-if="item.isShowMore" @click="showMoreTask(item)" class="tasksall">查看全部<i >》</i></div>
                <div v-if="item.isShowUnMore"  @click="unShowMoreTask(item)"class="tasksall icon">收取全部<i >》</i></div>

              </div>
            </div>
          </el-col>
        </el-row>

      </div>
    </div>

    <el-dialog
      class="group-class"
      :visible.sync="showLesson"
      width="11rem"
      :before-close="handleCloseModal"
    >
      <prepare-lesson :prepareFiles="prepareFiles"></prepare-lesson>
    </el-dialog>

    <open-class :classForPpt="classForStart" :type=type :class-chapter-id="classChapterId" ref="ClassBeginDialog" v-on:close="handleCommonClose"></open-class>

    <open-courseware :classForPpt="classForPpt" :class-chapter-id="classChapterId" ref="OpenCoursewarDialog" v-on:close="handleCoursewareClose"></open-courseware>

    <el-dialog v-if="isShowGroup"
      class="group-class"
      :visible.sync="isShowGroup"
      width="11rem"
      :close-on-click-modal="false"
      :before-close="handleCloseModal"
    >

      <group-class :chapter-groups-local="chapterGroups" :class-chapter-id="classChapterId" :chapter-groups-default="chapterGroupsDefault" :ungroup-students-local="ungroupStudents"  v-on:close="handleGroupClose" :ungroup-students-default="ungroupStudentsDefault"></group-class>
    </el-dialog>



    <prediction-task :my-survey-detail="mySurveyDetail"  :edit-able="editAble" :state.sync="isShowTask"  @completeTask="completeTask" v-on:close="handlePredictionClose"></prediction-task>

    <content-task-detail  :state="contentTaskShow"  @completeTask="completeTask" :task-result-id="taskresultId" v-on:close="handleContentTaskClose" 
    :contentTask.sync="contentTaskDetails"></content-task-detail ><!--以前注释过-->
    <!-- <content-task-detail  ></content-task-detail> -->

    <upload :state="uploadShow" :taskresult-id="taskresultId" @completeTask="completeTask" v-on:close="handleUploadClose" />

    <el-dialog
      class="group-class"
      :close-on-click-modal="false"
      :visible.sync="isShowStudentPreview"
      width="11.26rem"

      :before-close="handleCloseModal"
    >
      <student-preview :parent-class-course-id="classCourseId"></student-preview>

    </el-dialog>
      <!-- 优势打卡评价，已完成状态 弹框 -->
    <punch-card :state="isShowPanchCard"  :status="punchStatus" @close="parentClose"  :taskresultId="taskresultId"  :ATTENDEVAL="ATTENDEVAL"></punch-card>
  </div>
</template>

<script>

  import ContentTaskDetail from '../../../student/course/works/contentTaskDetail.vue'
  import PunchCard from "@/components/advantagePunchCard";
  import StudentPreview from '../preview'

  import PredictionTask from '../../../teachers/course/prediction'
  import Upload from '../../../student/course/works/upload.vue'

  import LoadingPage from '../../../common/loadingPage.vue'
  import { teacherPptAuthListApi } from 'api/api'
  import { teacherPptAuthApi } from 'api/api'


  import { teacherChapterStartListApi } from 'api/api'
  import { teacherChapterStartApi } from 'api/api'


  import { teacherChapterFilesApi } from 'api/api'

  import { teacherCourseDetailApi } from 'api/api'


  import { teacherGetChapterUnGroupApi } from 'api/api'

  import { teacherGetChapterGroupApi } from 'api/api'

  import { myTaskResultDetailApi } from 'api/api'
  import { myTaskResultAnswerDetailApi } from 'api/api'


  import { teacherSaveGroupApi } from 'api/api'


  import axios from 'axios'


  import breadcrumb from "@/components/common/breadcrumb.vue";
  import PopupModal from "@/components/popup";
  import PrepareLesson from "../prepareLesson";
  import OpenClass from "./openclass";
  import OpenCourseware from "./openCourseware";
  import GroupClass from "../groupClass";
  import breadcrumb_address from "assets/images/student/breadcrumb_address.png";
  import workimg from "assets/images/student/workimg.png";

  export default {
    name: "MyCourseView",
    components: {

      PredictionTask,
      ContentTaskDetail,
      Upload,
      StudentPreview,

      breadcrumb,
      PrepareLesson,
      OpenClass,
      OpenCourseware,
      PopupModal,
      GroupClass,
      LoadingPage,
      PunchCard
    },
    data() {
      return {
        isShowStudentPreview:false,
        isShowPanchCard:false,
        ATTENDEVAL:false,
        punchStatus:'2',
        showData:false,

        contentTaskShow:false,
        contentTaskDetails:false,
        isShowTask: false,
        myTasks:{},
        taskresultId:"",
        mySurveyDetail:{},
        editAble:false,
        uploadShow: false,

        showUploadButton:false,


        classChapterId:"123",
        classCourseId:"",
        courseDetail:{},

        classForPpt: {},
        classForPptShow: {},


        classForStart: {},
        classForPStartShow: {},


        ungroupStudents:{},
        ungroupStudentsDefault:{},


        chapterGroups:{},
        chapterGroupsDefault:{},



        prepareFiles: {},

        loading: true,
        workimg: "",
        computedData: [],
        // data: [1,2,3,4,5,6,7],
        desState: false,
        showLesson: false,
        showClass: false,
        isShowGroup: false,
        closeOverLay: false,
        visible: false,
        address: {
        },
        type:''
      };
    },
    created() {
       if(this.$route.query.type){
        this.type = this.$route.query.type
       }
      this.getCourseDetail()
      // this.computedtaskList();
    },
    filters:{
      filterWord(val){
        if(val && val.length>28){
          return val.substring(0,28)+'...'
        }else{
          return val
        }
      }
    },
    methods: {
      handleCoursewareClose(){
        this.getCourseDetail()
      },
      handleCommonClose(){
        this.getCourseDetail()
      },
      handleGroupClose(){
        this.isShowGroup = false
      },
      parentClose() {
        this.isShowPanchCard = false;
      },
      completeTask (taskresultId){

        let curChapters = this.courseDetail.classChapters
        for (let i = 0; i < curChapters.length; i++) {
          let curTasks = curChapters[i].taskShowList
          let innerBreak = false;
          if(curTasks){
            for (let j = 0; j < curTasks.length; j++) {
              let curTask = curTasks[j]
              if(curTask.taskresultId != null && curTask.taskresultId == taskresultId){

                curTask.completeStatus = '1'
                innerBreak = true
                break;
              }
            }
          }
          if(innerBreak){
            break;
          }
        }
      },
      changeContentTaskDetail(taskresultId){
        this.taskresultId = taskresultId;
        this.contentTaskDetails = true;
        this.contentTaskShow = !this.contentTaskShow;
      },
      handleContentTaskClose(){
        this.taskresultId = ""
        this.contentTaskShow = !this.contentTaskShow;
        // this.getCourseDetail()
      },
      handleUploadClose(){
        this.taskresultId = ""
        this.uploadShow = !this.uploadShow;
        // this.getCourseDetail()
      },
      handlePredictionClose(){
        this.mySurveyDetail = {}
        // this.getCourseDetail()
      },
      openNewPage(urlPath){

        let routeData = this.$router.resolve({
          path: urlPath
        });
        window.open(routeData.href, '_blank');
        //window.open(window.location.origin + '/courseware/'+item.id+'?type=studentTask')
      },

      goTrends(){
        this.$router.push('/teacher/task')
      },
      getCourseDetail(){
        this.showData = false
        let _this = this;
        this.classCourseId = this.$route.params.classCourseId
        let params = {
          classCourseId: this.classCourseId
        }
        axios.post(teacherCourseDetailApi, params,{
          headers: {
            'sid': localStorage.getItem("sid")
          }
        }).then(res => {
          let code = res.data.code
          if(code == 'success'){
            this.courseDetail = res.data.result
            this.address.text = this.courseDetail.classCourseName
            if(this.courseDetail.classChapters){
              for(let i=0; i<this.courseDetail.classChapters.length;i++){
                let taskList = this.courseDetail.classChapters[i].classChapterTaskInfoList
                if(taskList && typeof taskList =='string') taskList = this.deepCloneUtil(taskList)

                if(taskList && taskList.length > 3){
                  this.$set(this.courseDetail.classChapters[i],'isShowMore',true)
                  this.$set(this.courseDetail.classChapters[i],'isShowUnMore',false)

                  this.$set(this.courseDetail.classChapters[i], 'taskShowList',taskList.slice(0,3))
                } else {
                  this.$set(this.courseDetail.classChapters[i],'isShowMore',false)
                  this.$set(this.courseDetail.classChapters[i],'isShowUnMore',false)
                  this.$set(this.courseDetail.classChapters[i], 'taskShowList',taskList) 
                }
              }
            }
            this.showData = true
          } else {
            this.courseDetail = {}
          }
          setTimeout(() => {
            _this.loading = false;
          }, 500);
        })
      },
      openNewPage(urlPath){

        let routeData = this.$router.resolve({
          path: urlPath
        });
        window.open(routeData.href, '_blank');
        //window.open(window.location.origin + '/courseware/'+item.id+'?type=studentTask')
      },
      getMyTaskResult(taskResultId, editAble){
        this.loading = true;
        this.editAble = editAble
        let params = {
          id:taskResultId
        }
        // myTaskResultAnswerDetailApi
        if(editAble){

          axios.post(myTaskResultDetailApi, params,{
            headers: {
              'sid': localStorage.getItem("sid")
            }
          }).then(res => {

            let code = res.data.code
            if(code=='success'){
              // let myTasks = res.data.result.datalist
              this.mySurveyDetail = res.data.result

            } else {
              this.mySurveyDetail = {}
            }

            this.isShowTask = true

            setTimeout(() => {
              this.loading = false;
            }, 100);

          })
        } else {
          axios.post(myTaskResultDetailApi, params,{
            headers: {
              'sid': localStorage.getItem("sid")
            }
          }).then(res => {
            let code = res.data.code
            if(code=='success'){
              // let myTasks = res.data.result.datalist
              this.mySurveyDetail = res.data.result

            } else {
              this.mySurveyDetail = {}
            }
            this.isShowTask = true
            setTimeout(() => {
              this.loading = false;
            }, 100);

          })
        }
      },
      showDetail(item,coursewareStatus) {
        console.log(item)
        if(item.acttype == 'TEST'){
          this.getMyTaskResult(item.id,false)

        } else if(item.acttype == 'QA'){
          this.openNewPage("/questionnaire/teacher/" + item.id)
        } else if(item.acttype == 'BEST'){
          // if(coursewareStatus=='1'){
            this.taskresultId = item.id;
            this.isShowPanchCard = true;
            this.ATTENDEVAL = false;
            this.punchStatus = '3';
          // }
        } else if(item.acttype == 'CONTENT'){
         // this.changeContentTaskDetail(item.courseTask.id) 这是以前判断
        console.log(item.id)
          this.changeContentTaskDetail(item.id)
        } else if(item.acttype == 'WORKSUPLOAD'){
          this.changeUpload(item.id)
        } else if(item.acttype == 'ATTENDEVAL'){
        }
      },
      getPrepareFiles(classChapterId){
        this.prepareFiles = {}
        let params = {
          classChapterId: classChapterId
        }
        this.classChapterId = classChapterId
        axios.post(teacherChapterFilesApi, params,{
          headers: {
            'sid': localStorage.getItem("sid")
          }
        }).then(res => {
          let code = res.data.code
          if(code == 'success' && res.data.result){
            this.prepareFiles = res.data.result.datalist||[]

            for(let i=0;i<this.prepareFiles.length;i++){
              this.$set(this.prepareFiles[i],'isHover',false)
            }
          } else {
            this.prepareFiles = {}
          }
          this.showLesson = !this.showLesson;
        })
      },
      getClassForStart(classChapterId){
        this.classForStart = []
        this.classForStartShow = {}

        let params = {
          classChapterId: classChapterId
        }

        this.classChapterId = classChapterId

        axios.post(teacherChapterStartListApi, params,{
          headers: {
            'sid': localStorage.getItem("sid")
          }
        }).then(res => {
          let code = res.data.code
          if(code == 'success'){
            this.classForStart = res.data.result.datalist
          } else {
            this.classForStart = []
          }
          this.$refs.ClassBeginDialog.show();
        })
      },
      getClassForPpt(classChapterId){
        this.classForPpt = {}
        this.classForPptShow = {}

        let params = {
          classChapterId: classChapterId
        }

        this.classChapterId = classChapterId

        axios.post(teacherPptAuthListApi, params,{
          headers: {
            'sid': localStorage.getItem("sid")
          }
        }).then(res => {

          let code = res.data.code

          if(code == 'success'){
            this.classForPpt = res.data.result.datalist
          } else {
            this.classForPpt = {}
          }

          this.$refs.OpenCoursewarDialog.show();
        })
      },
      showMoreTask(item){
        let taskList = item.classChapterTaskInfoList

        if(taskList && taskList.length > 3){
          item.isShowMore = false
          item.isShowUnMore = true
          item.taskShowList = taskList
        }
      },
      unShowMoreTask(item){
        let taskList = item.classChapterTaskInfoList

        if(taskList && taskList.length > 3){
          item.isShowMore = true
          item.isShowUnMore = false
          item.taskShowList = taskList.slice(0,3)
        }
      },
      handleCloseModal(done) {
        done()
        // this.getCourseDetail()
      },
      handlePrepareLesson(classChapterId) {
        this.getPrepareFiles(classChapterId)
      },
      handleBeginClass(classChapterId) {
        this.classChapterId = classChapterId
        this.getClassForStart(classChapterId)
      },
      handleOpenCourseware(classChapterId) {

        this.getClassForPpt(classChapterId)
      },
      handleGrouping(classChapterId) {
        // this.getUngroupStudent(classChapterId)
        this.classChapterId = classChapterId

        setTimeout(() => {
          this.isShowGroup = !this.isShowGroup
        }, 100);
      },

      changeUpload(taskresultId){
        this.taskresultId = taskresultId
        this.uploadShow = !this.uploadShow;
      },
      fitlerdes() {
        let text = this.courseDetail.classCourseDesc + "";
        if(text.length >　135){
          return this.desState ? text : text.slice(0, 133) + '...'
        } else {
          return text;
        }
      },
      searchMore(item) {
        if (!item.showText) {
          return;
        }
        let list = this.computedData;
        for (let i = 0; i < list.length; i++) {
          if (list[i].id === item.id) {
            if (item.showText === "查看全部") {
              list[i].taskListStr = list[i].taskList;
              list[i].showText = "收取全部";
            } else {
              list[i].taskListStr = list[i].taskList.slice(0, 3);
              list[i].showText = "查看全部";
            }
          }
        }
        this.computedData = list;
      }
    }
  };
</script>

<style lang="scss" scoped>


  .TEST-task {
    height: .16rem;
    text-indent: .25rem;
    background-size: contain;
    background: url("../../../../assets/images/icon/task1.png")  0 0 no-repeat;
  }
  .QA-task {
    height: .16rem;
    text-indent: .25rem;
    background-size: contain;
    background: url("../../../../assets/images/icon/task2.png")  0 0 no-repeat;
  }
  .BEST-task {
    height: .16rem;
    text-indent: .25rem;
    background-size: contain;
    background: url("../../../../assets/images/icon/task3.png")  0 0 no-repeat;
  }
  .CONTENT-task {
    height: .16rem;
    text-indent: .25rem;
    background-size: contain;
    background: url("../../../../assets/images/icon/task4.png")  0 0 no-repeat;
  }
  .WORKSUPLOAD-task {
    height: .16rem;
    text-indent: .25rem;
    background-size: contain;
    background: url("../../../../assets/images/icon/task5.png")  0 0 no-repeat;
  }

  .mycourseview {
  height: calc(100% - 0.1rem);
  .view {
    margin-top: 0.15rem;
    height: calc(100% - 0.29rem);
    overflow-y: scroll;
    overflow-x: hidden;
    .des,
    .li {
      margin: 0;
      background: #fff;
      border: 0.01rem solid rgba(228, 232, 237, 1);
      border-radius: 0.05rem;
      padding: 0.2rem 0.1rem;
      margin-bottom: 0.1rem;
    }
    .workimg {
      width: 1.88rem;
      height: 1.98rem;
      margin: 0 auto;
      img{
        width:100%;
        height:100%;
      }
    }
    .workdes {
      margin-right: 0.2rem;
      h3 {
        font-size: 0.2rem;
        color: rgba(51, 51, 51, 1);
        margin-top: 0.15rem;
      }
      p {
        font-size: 0.14rem;
        color: rgba(136, 136, 136, 1);
        line-height: 0.24rem;
        margin-top: 0.25rem;
        var {
          color: #f79727;
          cursor: pointer;
          margin-top: 0.1rem;
        }
      }
      div {
        background: rgba(245, 246, 247, 1);
        border-radius: 0.02rem;
        padding: 0.05rem 0.1rem;
        margin-top: 0.15rem;
        span {
          display: block;
          color: rgba(51, 51, 51, 0.8);
          margin: 0.1rem 0 0.1rem 0.13rem;
          font-size: 0.14rem;
          line-height: 0.2rem;
          text-indent: 0.09rem;
          position: relative;
          &:after {
            content: "";
            display: block;
            width: 0.04rem;
            height: 0.04rem;
            background-color: #f79727;
            position: absolute;
            left: 0;
            top: 0.07rem;
          }
        }
      }
      var {
        cursor: pointer;
      }
      .previewstudents {
        text-indent: 0.15rem;
        display: inline-block;
        position: absolute;
        right: 0.4rem;
        top: 0.15rem;
        color: #f79727;
        font-size: 0.16rem;
        &:after {
          display: block;
          content: "";
          border-width: 0.06rem;
          border-style: solid;
          border-color: transparent transparent transparent #f79727;
          position: absolute;
          left: 40%;
          top: 100%;
          position: absolute;
          left: 0;
          top: 0.02rem;
        }
      }
    }

    .workList {
      height: 3.6rem;
      .workList-item {
        min-height: 2.14rem;
        &>.li {
          min-height: 2.14rem;
        }
      }
      .li {
        padding: 0.15rem;
      }
      h4 {
        font-size: 0.16rem;
        color: rgba(51, 51, 51, 1);
        text-indent: 0.15rem;
        position: relative;
        margin-bottom: 0.2rem;
        font-weight: bold;
        &:after {
          content: "";
          display: block;
          width: 0.05rem;
          height: 0.05rem;
          background-color: #f79727;
          border-radius: 100%;
          position: absolute;
          left: 0;
          top: 0.06rem;
        }
        // i {
        //   width: 0.14rem;
        //   height: 0.16rem;
        //   display: inline-block;
        //   margin-left: 0.1rem;
        //   background: url(data:image/png;base64,)
        //     top left no-repeat;
        //   background-size: 100% 100%;
        // }
      }
      .workdes {
        background: rgba(255, 248, 240, 1);
        padding: 0.2rem 0.2rem 0.15rem 0.2rem;
        border-radius: 0.03rem;
        width: 100%;
        span {
          height: .15rem;
          font-size: 0.14rem;
          color: rgba(51, 51, 51, 1);
          cursor: pointer;
          display: flex;
          text-indent: 0.25rem;
          line-height:0.15rem;
          background: url(data:image/png;base64,)
            top left no-repeat;
          background-size: contain;
        }
        var,
        b {
          font-size: 0.14rem;
          color: rgba(153, 153, 153, 1);
          margin-top: 0.25rem;
          display: inline-block;
          cursor: pointer;
        }
        var{
          margin-top: 0.3rem;
        }
        i {
          width: 0.14rem;
          height: 0.16rem;
          display: inline-block;
          margin-left: 0.1rem;
          background: url(data:image/png;base64,)
            top left no-repeat;
          background-size: 100% 100%;
        }
      }
      .myteam {
        margin: 0.15rem 0 0 0;
        overflow: hidden;
        cursor: pointer;
        span {
          font-size: 0.14rem;
          color: rgba(102, 102, 102, 1);
          float: left;
          &.i {
            text-indent: 0.25rem;
            margin-top: 0.1rem;
            background: url(data:image/png;base64,)
              top left no-repeat;
          }
        }
        .classdata,
        .classstart {
          color: rgba(153, 153, 153, 1);
          float: right;
          width: 1.1rem;
          height: 0.3rem;
          border-radius: 0.16rem;
          line-height: 0.3rem;
          text-align: center;
          font-size: 0.12rem;
          position: relative;
          text-indent: 0.2rem;
        }
        .classdata {
          color: #f79727;
          border: 0.01rem solid rgba(247, 151, 39, 1);
          &:after {
            content: "";
            position: absolute;
            width: 0.15rem;
            height: 0.18rem;
            top: 0.05rem;
            left: 0.2rem;
            background-image: url(data:image/png;base64,);
          }
        }
        .classstart {
          color: #fff;
          background: rgba(247, 151, 39, 1);
          margin-left: 0.15rem;
          text-indent: 0.3rem;
          &:after {
            content: "";
            position: absolute;
            width: 0.21rem;
            height: 0.18rem;
            top: 0.05rem;
            left: 0.15rem;
            background-image: url(data:image/png;base64,);
            background-size: contain;
          }
        }
      }

			.tasks {
				border-top: #E4E8ED 0.01rem solid;
				margin: 0 -0.15rem;
				ul {
					margin: 0.1rem 0.1rem 0 0.1rem;
				}
				li {
					padding: 0.1rem 0.2rem;
					cursor: pointer;
					a {
						color: #666666;
					}
					&:hover {
						background: rgba(245,246,247,1);
						a {color: #F79727}
						span{
							color:rgba(247,151,39,1);
						}
					}
					.active {color: #999}
					.el-col-6 {
						text-align: right;
					}
					small {
						color: #888;
						font-size: 0.12rem;
					}
					.li1 {
						height: .16rem;
						text-indent: 0.25rem;
						background: url(data:image/png;base64,) top left no-repeat;
					}
					.li2 {
						text-indent: 0.25rem;
						height: .16rem;
						background: url(data:image/png;base64,) top left no-repeat;
					}
					.li3 {
						text-indent: 0.25rem;
						height: .16rem;
						background: url(data:image/png;base64,) top left no-repeat;
					}
				}
				.tasksall {
          text-align: center;
          color: #f79727;
          margin-top: 0.1rem;
          cursor: pointer;
          i {
            transform: rotate(90deg);
            display: inline-block;
            margin: 0.03rem 0 0 0.05rem;
            vertical-align: middle;
          }
          &.icon {
            i {
              transform: rotate(-90deg);
              margin: -0.08rem 0 0 0.05rem;
            }
          }
          &.taskNo {
            height: 0.16rem;
          }
        }
        .taskNoMore {
          height: 0.17rem;
        }
      }

      .nowork {
        // height: 2.5rem;
        height: 2.14rem;
        position: relative;
        h4 {
          color: #333;
        }
        .nowork_myteam {
          margin-top: 1.35rem;
          position: relative;
          z-index: 100;
        }
        .icon_no {
          position: absolute;
          left: 0;
          top: 0;
          width: 100%;
          height: 100%;
          background: rgba(255, 255, 255, 0.8);
          border-radius: 0.05rem;
          div {
            width: 0.75rem;
            margin: 0.7rem auto 0;
          }
          i {
            height: 0.46rem;
            width: 0.46rem;
            border: #ccc 0.03rem solid;
            border-radius: 100%;
            display: inline-block;
            position: relative;
            &:before {
              content: "";
              position: absolute;
              top: 52%;
              background: #ccc;
              width: 105%;
              height: 0.03rem;
              transform: rotate(-30deg);
            }
          }
          span {
            color: #ccc;
            font-weight: bold;
            display: block;
            margin: 0.1rem 0 0 -0.1rem;
          }
        }
      }
    }
  }
}

.group-class /deep/ .el-dialog__header {
  padding: 0;
}

.group-class /deep/ .el-dialog__body {
  padding: 0;
}
</style>
