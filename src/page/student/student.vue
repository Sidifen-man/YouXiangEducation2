  ...
  <div class="g_main">
     <section class="main">
      <aside>
        ...
        <p>{{className}}</p>
        <router-link style="cursor: pointer;" to="/student/course/mycourse" tag="div" :class="[{'active':isActive===1},'nav']">
          <span><img src="../../assets/images/icon/icon_course.png" alt=""></span>
          <span>课程中心</span>
        </router-link>
        <router-link style="cursor: pointer;" to="/student/task" tag="div" :class="[{'task':isActive===2},'nav']">
...
export default {
  ...
  watch: {
    "$route.path"(newVal, oldVal) {
      if (newVal.includes("/student/task")) {
        this.isActive = 2;
  ...
  created() {
    ...
    if (curuser.classInfo) this.className = curuser.classInfo.name
