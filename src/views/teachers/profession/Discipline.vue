  ...
  <div class="profession">
    <div class="category">
      ...
      <div class="list">
        ...
        <ul>
          <li class="menu"  :class="{active: current.index == item.id}"  v-for="item in list" :key="item.id"  @click="onTab(item.id)" >
            <p>{{item.name}}</p>
            <div  class="sub-menu"  v-show="item.sub_menu&&current.index==item.id&&item.sub_menu.length > 0&&item.is_show" >
              <ul>
                <li v-for="theme in item.sub_menu" :key="theme.id" :class="{'active': current.sub_index == theme.id,'overflow1':true}"
                  @click.stop="select(theme.id)">{{theme.name}}</li>
              </ul>
            </div>
          </li>
        </ul>
    ...
    <div class="pro-list">
      <div style="padding: 0 14px;">
        <div class="pro-search">
          <input class="ser-input" placeholder="请输入职业名称"/>
        ...
        <div class="tag-category">
          <div class="common-item flexCenterY">
            <div class="collect">
              ...
              <span @click="changeCollect(i)" :class="{'active': collIdx==i}" v-for="(v, i) in collection" :key="i" >{{v}}</span>
            </div>
          </div>
          <div class="common-item search-job flexCenterY">
           <div class="hot">
            ...
            <div class="tag-btn">
              <el-button class="custom"  @click="search"><img src="../../../images/global/a_search.png" alt="" class="imgIcon">查询</el-button>
              <el-button class="custom"  @click="reset" ><img src="../../../images/global/a_reset.png" alt="" class="imgIcon"> 重置</el-button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="work-list">
        <div>
          <ul class="work-con">
            <li v-for="item in profession" :key="item.id" >
              <div class="work-cover">
                <div class="label-button">
                  <span class="love" :class="{active: item.is_collect}"  @click="handleLove(item,index)"></span>
                </div>
                <img class="fengmian" src="../../../images/global/discipline-1.png" alt v-if="item.id%3==1"/>
                <img class="fengmian" src="../../../images/global/discipline-2.png" alt v-if="item.id%3==2" />
                <img  class="fengmian" src="../../../images/global/discipline-3.png" alt v-if="item.id%3==0" />
              </div>
        ...
        <!-- 分页部分 -->
        <el-pagination  class="page"  background layout="prev, pager, next"  :total="count" ></el-pagination>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    start_detail(id) {
      if (this.selectVal === '海外大学专业库') {
        this.$router.push(`/teacher/profession/professionDetail/1/${id}`)
      } else if (this.selectVal === '国内大学专业库') {
        this.$router.push(`/teacher/profession/professionDetail/2/${id}`)
      }
    },
