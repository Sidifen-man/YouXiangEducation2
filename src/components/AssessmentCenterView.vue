...
export default {
  ...
  methods: {
    ...
    readMore() {
      this.$router.push({
        path: "/students/home/readMore",
        query: { type: 1 }
      });
    },
    evalution() {
      this.$router.push({
        path: "/students/home/evaluate"
      });
    },
    next(i) {
      if (i <= 2) {
    ...
    strengthportrait() {
      ...
      let id = ''
      if(grade.classInfo.classType === "1")id = 'via86'
      if(grade.classInfo.classType === "2")id = 'via85';
      let param = {
        id: id,
        lang: lang
      };
